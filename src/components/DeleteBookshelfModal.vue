  ...
  <div v-if="isVisible" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center flex justify-center items-center bg-black">
    <div class="bg-white p-4 rounded-lg w-[300px] z-50">
      ...
      <div class="flex justify-end gap-2">
        <button @click="$emit('closeModal')" class="bg-gray-400 px-3 py-1 rounded cursor-pointer">Cancel</button>
        <button @click="$emit('deleteBookshelf')" class="bg-red-600 text-white px-3 py-1 rounded cursor-pointer">Delete</button>
      </div>
    </div>
  </div>
</template>
