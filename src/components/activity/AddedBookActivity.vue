            ...
            <div class="flex-shrink-0">
                <div class="cursor-pointer" @click="$router.push({ name: 'profilepage', params: { id: user.id } })">
                    ...
                    <img v-else src="@/assets/default_profile_picture.jpg" alt="User profile image"
                        class="w-20 h-20 rounded-full text-inherit object-cover border" />
                </div>
            </div>
            <div>
                <p class="text-sm text-gray-500">{{ new Date(activity.createdAt).toLocaleDateString() }} at {{ new
                    Date(activity.createdAt).toLocaleTimeString() }}</p>
                <p class="mt-1">
                    <span @click="$router.push({ name: 'profilepage', params: { id: user.id } })"
                        class="font-bold text-black cursor-pointer">{{ user.username }}</span>
