    ...
    <div class="h-screen overflow-y-auto bg-white text-black">
      <div class="container mx-auto p-4">
          ...
          <div class="space-y-6 lg:col-span-1">
            ...
            <div>
              ...
              <div v-for="progress in readingProgressList" :key="progress.id" class="mb-4">
                <ReadingProgress :id="progress.id" :currentPage="progress.currentPage" :totalPages="totalPages || 0" />
        ...
        <EditProgressModal v-if="selectedProgress" :visible="isModalOpen" :progressId="selectedProgress.id"
          :currentPage="selectedProgress.currentPage" :totalPages="selectedProgress.book?.numberOfPages || 0"
...
async function updateProgress(payload: { progressId: string; currentPage: number }) {
  if (!userId.value) return;
  try {
    await axios.put(`/api/Stats/${payload.progressId}/edit`, {
      id: payload.progressId,
      currentPage: payload.currentPage,
    });

    // Update local state
    const progress = readingProgressList.value.find(p => p.id === payload.progressId);
    if (progress) progress.currentPage = payload.currentPage;
